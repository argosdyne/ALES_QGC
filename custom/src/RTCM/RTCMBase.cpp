...
void RTCMBase::handle_gps_raw_int_status(const mavlink_message_t &message)
{
    ...
    qInfo() << "gpggaFromVehicle: " << _gpggaFromVehicle;
...
{
    qWarning() << "RTCM: no MAVLink GPS_RTCM_DATA sent for 2s — streaming flag cleared (vehicle RTK may drop if corrections stopped)";
    setStreaming(false);
}
