...
//    }
    ...
    // Hidden service gesture: 3-second press on the bottom-right corner opens the system
    // WiFi panel (WiFi ON + connect to networks) without exiting kiosk.
    MouseArea {
        id: wifiHiddenGestureArea
        anchors.right: parent.right
        anchors.bottom: parent.bottom
        anchors.rightMargin: _toolsMargin
        anchors.bottomMargin: _toolsMargin
        ...
        Timer {
            ...
            repeat: false
            onTriggered: {
                if (!wifiHiddenGestureArea.holding) {
                    return
                }
                var now = Date.now()
                if (now - wifiHiddenGestureArea.lastTriggerMs < 10000) {
                    return
                }
                wifiHiddenGestureArea.lastTriggerMs = now
                var opened = CustomQmlInterface.requestDpcOpenWifiPanel()
                CustomQmlInterface.showMessage(
                    opened ? qsTr("WiFi settings opened — connect to a network, then close the panel.")
                           : qsTr("Could not open WiFi settings on this device."),
                    opened ? 3 : 1)
            }
        }
