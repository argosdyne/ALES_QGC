...
Item {    
    ...
    ColumnLayout {
        ...
        GridLayout {
            ...
            QGCLabel {
                //text: (_activeVehicle && _activeVehicle.gimbalData ? _activeVehicle.gimbalPitch.toFixed(0) : "0") + "Â°"
                text: {
                    var pitch = (_activeVehicle && _activeVehicle.gimbalData)
                            ? _activeVehicle.gimbalPitch
                            : 0
                    pitch = Math.abs(pitch) < 0.5 ? 0 : pitch
                    return pitch.toFixed(0) + "Â°"
                }
                Layout.fillWidth: true
                horizontalAlignment: Text.AlignHCenter
                font.pixelSize: ScreenTools.defaultFontPixelHeight
            ...
            QGCLabel {
                //text: (_activeVehicle && _activeVehicle.gimbalData ? _activeVehicle.gimbalYaw.toFixed(0) : "0") + "Â°"
                text: {
                    var yaw = (_activeVehicle && _activeVehicle.gimbalData)
                            ? _activeVehicle.gimbalYaw
                            : 0
                    yaw = Math.abs(yaw) < 0.5 ? 0 : yaw
                    return yaw.toFixed(0) + "Â°"
                }
                wrapMode: Text.WordWrap
                Layout.fillWidth: true
                horizontalAlignment: Text.AlignHCenter
