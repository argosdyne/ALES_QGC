...
// Self-contained NextVision DragonEye2 control panel. It owns its own payload
// controller (RC_CHANNELS_OVERRIDE to the gimbal's ArduPilot over UDP) and does
// NOT depend on the active vehicle.
Rectangle {
    id:             root
    ...
    NextVisionPayloadController { id: payload }
    ...
    Column {
        ...
        QGCLabel {
            width:                  parent.width
            text:                   qsTr("NextVision DragonEye2")
            font.bold:              true
        ...
        QGCLabel {
            width:                  parent.width
            horizontalAlignment:    Text.AlignHCenter
            text:                   payload.connected ? qsTr("Streaming override") : qsTr("Not connected")
        ...
        Grid {
            ...
            QGCButton {
                width: directionGrid.buttonWidth; height: directionGrid.buttonHeight
                text: qsTr("STOP")
                onClicked: { root._up = root._down = root._left = root._right = false; payload.gimbalStop() }
            }
            QGCButton {
                width: directionGrid.buttonWidth; height: directionGrid.buttonHeight
                text: qsTr("RIGHT")
