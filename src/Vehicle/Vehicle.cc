...
void Vehicle::sendJoystickDataThreadSafe(float roll, float pitch, float yaw, float thrust, quint16 buttons, float auxPitch, float auxRoll)
{
    ...
    // Some firmware/bridges only apply MANUAL_CONTROL to the first 8 RC channels.
    // Send dedicated RC override values for channels 9/10 when dial axes are configured.
